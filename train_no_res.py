...
def train(
    ...
    output_path = os.path.join(output_dir, "no_res.txt")
    ...
    # Setup optimizer
    param_list = (
        list(model.parameters()) + list(model2.parameters())
    )
    ...
    for epoch in range(num_epochs):
        ...
        train_loss = train_epoch(
            model, model2, dataloader_train, optimizer, criterion
        )

        # Validation phase
        val_loss = evaluate(model, model2, dataloader_val, criterion)
    ...
    test_results = test_models(
        model, model2, dataloader_test, criterion, base_dir
    )
...
def train_epoch(model, model2, dataloader, optimizer, criterion):
...
def evaluate(model, model2, dataloader, criterion):
...
def test_models(model, model2, dataloader, criterion, base_dir):
...
def save_visualizations(x, y, outputs, idx, base_dir):
    ...
    under_sampling_filename = os.path.join(
        base_dir, "under_sampling", f"under_sampling_{idx}.png"
    )
    full_sampling_filename = os.path.join(
        base_dir, "full_sampling", f"full_sampling_{idx}.png"
    )
    reconstruction_filename = os.path.join(
        base_dir, "reconstruction", f"reconstruction_{idx}.png"
    )
